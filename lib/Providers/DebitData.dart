...
class DebitData with ChangeNotifier {
  ...
  final String baseUrl = 'http://192.168.1.12:9090/api/debit';
  //10.0.2.2:9090
  ...
  Future<void> deleteDebit(int transactionId) async {
    ...
    var index =
        _debits.indexWhere((element) => element.transactionId == transactionId);
  ...
  Future<void> updateDebit(Debit debit, int transactionId) async {
    ...
    try {
      await http.put(url,
          ...
          body: jsonEncode({'amount': debit.amount}));
      for (var element in _debits) {
